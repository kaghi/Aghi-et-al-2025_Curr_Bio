...
for AZ_num = 223:15 %whatever AZ or index you want (see note above for Ib vs Is ranges)
    pre_traces = []
    for episode_num = 1:NumIndices %number of indices. variable should be in ImageAnalysis Setup which we loaded in earlier
...
%For 5 Hz
xlimmy = length(QuaSOR_Matched_AZ_Structure.All_Boutons(AZ_num).DeltaFF0_MeanTrace);
...
y = QuaSOR_Matched_AZ_Structure.All_Boutons(AZ_num).DeltaFF0_MeanTrace
x = [1:length(y)];
    ylabel('DeltaF/F0')
    plot(x,y)
    box on
