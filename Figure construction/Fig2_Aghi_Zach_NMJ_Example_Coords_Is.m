...
BoutIsInd = find(BoutType(:,1)==2);
AZCoordXIs = AZCoordX(1:length(BoutIsInd),1);
AZCoordYIs = AZCoordY(1:length(BoutIsInd),1);

Fig2SampleCoordsX = AZCoordXIs;
Fig2SampleCoordsY = AZCoordYIs;
Fig2SampleCoordsLFPr = ExperimentSet_Reduced(3).Grouped_Data_Reduced(AnimalNo).Verified_Quantifications.BoutonSorted(2).All_QuaSOR_Data(4).Recording(1).Evoked_Pr;
